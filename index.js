...
app.post('/newsfinder', (req, res) => {
  ...
  newsapi.v2.everything({
    ...
    if(news.totalResults == 0) res.render("response", { formattedNews: "No relevant articles found.", title: "News Finder" });
      ...
      const prompt = "Organize and elaborate these news. Combine news from all available sources to eliminate bias, if multiple sources are available. If multiple languages are present, translate everything to English, otherwise everything must be in it's source language. " + prepareNewsPrompt(news);
      const newsSummary = await model.generateContent(prompt);
      const formattedNews = marked(newsSummary.response.text());
